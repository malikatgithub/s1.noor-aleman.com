@extends('layouts.css_main')

@section('body')

@include('layouts.teacher_nav')

@include('layouts.top-menu')
    <div class="container mt-5">
        <div class="row justify-content-center">
            <div class="col-md-12 text-right">

                 {{-- Start of card Div  --}}

                 <div class="card border-top-0">
                        <div class="card-header border-top1">
                          تعديل بيانات مدرس 
                        </div>
                            <br>
                            <div class="container">
                {{-- Start of form  --}}
                            <form action="{{route('teacher.update', ['id' => $teacher->id ])}}" method="POST" enctype="multipart/form-data">
                                {{@csrf_field()}}
                                <div class="card flex-row flex-wrap">
                                      
                                    <div class="card-header w-100 text-muted border-top1 font-weight-bold mb-3">
                                          -   بيانات المدرس 
                                         
                                    </div>
                                    
                                
                                    <div class=" border-0 p-2">
                                    
                                            <img src="{{asset("public/$teacher->pic")}}" alt="" width="150px" height="150px" class="img-thumbnail">
                                     
                                        
                                    </div>


                                    {{-- Row of teacher information --}}
                                <div class=" border-0 flex-column" style="width:82.2%;">           

                                    <div class="row ">
                                        <div class="col-md-4 mb-4">
                                                <label for="validationDefault01">الإسم<span class="required">*</span> </label>
                                                <input type="text" class="form-control" id="validationDefault01" placeholder=""  required name="name" value="{{$teacher->name}}">
                                        </div>
                                        
                                        <div class="col-md-4 mb-4">
                                                <label for="validationDefault01">المؤهل الأكاديمي<span class="required">*</span></label>
                                                        <select class="custom-select" required name="qualification">
                                                                <option value=""></option>
        
                                                                @if ($teacher->qualification =='بكالوريوس')
                                                                <option value="بكالوريوس" selected>بكالوريوس</option>
                                                                <option value="ماجستير" >ماجستير</option>
        
                                                                @else
                                                                <option value="ماجستير" selected>ماجستير</option>
                                                                <option value="بكالوريوس" >بكالوريوس</option>
        
                                                                @endif
        
                                                        </select>
                                        </div>

                                        <div class="col-md-4 mb-4">
                                                <label for="validationDefault01">تاريخ الميلاد <span class="required">*</span></label>
                                                <input type="date" class="form-control" id="validationDefault01" placeholder="" value="{{$teacher->dob}}" required name="dob">
                                        </div>
                                        
                                        
                                </div>
                                

                                <div class="row ">
                                        <div class="col-md-3 mb-3">
                                                <label for="validationDefault01">الجنس <span class="required">*</span></label>
                                                <select class="custom-select" required name="gender">
                                                        <option value=""></option>

                                                        @if ($teacher->gender =='ذكر')
                                                        <option value="ذكر" selected>ذكر</option>
                                                        <option value="أنثي" >أنثي</option>

                                                        @else
                                                        <option value="أنثي" selected>أنثي</option>
                                                        <option value="ذكر" >ذكر</option>

                                                        @endif

                                                </select>
                                        </div>   


                                        <div class="col-md-3 mb-3">
                                                <label for="validationDefault01">الهاتف</label>
                                                <input type="text" class="form-control" id="validationDefault01" placeholder="إختياري" value="{{$teacher->phone}}"  name="phone" >
                                        </div>

                                        <div class="col-md-3 mb-3">
                                                <label for="validationDefault01">رقم الهوية<span class="required">*</span></label>
                                                <input type="number" class="form-control" id="validationDefault01" placeholder="" value="{{$teacher->id_card}}" required name="id_card" >
                                        </div>

                                        <div class="col-md-3 mb-4">
                                                <label for="validationDefault01"> المرتب <span class="required">*</span></label>
                                                <input type="number" class="form-control" id="validationDefault01" placeholder="المرتب الشهري" value="" required  name="salary" {{ $teacher->salary }}>
                                        </div>

                            
                                </div>

                                <div class="row">

                                    <div class="col-md-3 mb-3">
                                            <label for="validationDefault01">تاريخ الإنضمام  <span class="required">*</span></label>
                                            <input type="date" class="form-control" id="validationDefault01" placeholder="" value="{{$teacher->join_date}}" required name="join_date">
                                    </div>

                                    <div class="col-md-3 mb-3">
                                            <label for="validationDefault01"> عنوان السكن <span class="required">*</span></label>
                                            <input type="text" class="form-control" id="validationDefault01" placeholder="" value="{{$teacher->address}}" required name="address">
                                    </div>

                                    <div class="col-md-6 mb-3">
                                            <label for="validationDefault01">الإيميل</label>
                                            <input type="email" class="form-control" id="validationDefault01" placeholder="إختياري" value="{{$teacher->email}}"  name="email" >
                                    </div>

                                     <div class="col-md-3 mb-3">

                                            <label for="validationDefault01">تحميل صورة</label>
                                            <input type="file" class="form-control-file" id="validationDefault01" placeholder="إختر الصورة" value="{{$teacher->pic}}"  name="pic">
                                            
                                     </div>


                                     <div class="col-md-3 mb-3">

                                                <div class="form-check text-danger font-weight-bold">
                                                        <input class="form-check-input border border-danger" type="checkbox" value="default.png" id="defaultCheck1" name="delete_pic">
                                                        <label class="form-check-label" for="defaultCheck1">
                                                                <br>
                                                                حذف الصورة
                                                        </label>
                                                </div>
                                     </div>
                   
                                </div>
                                

                                <div class="row ">
                                    <div class="col-md-12 mb-12">
                                            <div class="form-group">
                                                    <label for="exampleFormControlTextarea1">ملاحظات</label>
                                                    <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" placeholder="" name="teacher_note">{{$teacher->teacher_note}}</textarea>
                                            </div>
                                    </div>
                                        
                                </div>

        
                         
                                
                            </div>
                        </div>

                          {{-- Row of teacher information --}}

                     
                        
                                         
                
                                        <br>
                                        
                                        <center>
                                            <button type="submit" class="btn btn-success">حفظ البيانات</button>
                                        </center>
                                       
                                    </form>
                                    {{-- End of form of card Div  --}}

                                    <br>
                                    <br>
                            </div>
                           
                        
    
    
                    </div>
    
                    {{-- End of Start of card Div  --}}

            </div>
        </div>
    </div>

   
@endsection
