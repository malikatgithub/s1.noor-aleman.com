    ...
    <div class=" pt-3">
        ...
        <ul class="m-4"> 
            ...
            <li><h6>
                ...
                تاريخ السلفيات : {{$date}}
                @else
                تاريخ السلفيات : {{$start_date}} - {{$end_date}}
                @endif
                {{-- # FUNCTION FOR RETREVE THE DATE  --}}                
            </h6></li>
        </ul>
        @if (isset($expense_info))
            {{-- VERFY THE EXPENSES FOR SPECIFIC DATE WHERE $EXPENSES_INFO IS NULL = 00 --}}
                ...
                @if ($info == 00)
                <div class="alert alert-dark mt-3">
                        <center><h5 class="text-center font-weight-bold">لاتوجد سلفيات بهذا التاريخ</h5> </center>
                </div>
                @endif
                            ...
                            @foreach ($expense_info as $expense)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        {
                                            $data = Employees::find($expense->employee_id);
                                            echo $data->name;
                                        }
                                        else{
                                            $data = Teachers::find($expense->teacher_id);
                                            echo $data->name;
                                            }
                                        ?>
